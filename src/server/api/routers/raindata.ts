...
export const rainDataRouter = createTRPCRouter({
  ...
  testIHist: publicProcedure.query(async () => {
    const sqlString = `
      SELECT
        timestamp,
        ADAMS.AF2295LQT.F_CV.VALUE,
        ADAMS.AF2295LQY.F_CV.VALUE
      FROM IHTREND
  ...
  currentValues: publicProcedure.query(async () => {
    const queryString = `
      SELECT TOP 1
        timestamp,
        ADAMS.AF2295LQT.F_CV.VALUE,
        FOURCHE.FC2295LQT.F_CV.VALUE, 
        ADAMS.CAB2295LQT.F_CV.VALUE, 
        ADAMS.AS1941CAT.F_CV.VALUE, 
        ADAMS.CR1941LQT.F_CV.VALUE, 
        ADAMS.CV1942CAT.F_CV.VALUE, 
        ADAMS.HR1942CAT.F_CV.VALUE, 
        ADAMS.JR1941CAT.F_CV.VALUE, 
        MAUMELLE.LM1941CAT.F_CV.VALUE,
        ADAMS.RR1942CAT.F_CV.VALUE, 
        ADAMS.LF1941CAT.F_CV.VALUE, 
        ADAMS.OC1941CAT.F_CV.VALUE, 
        ADAMS.PF2295LQT.F_CV.VALUE, 
        ADAMS.TS1941CAT.F_CV.VALUE, 
        ADAMS.CM1942CAT.F_CV.VALUE, 
        ADAMS.SW1942CAT.F_CV.VALUE, 
        ADAMS.SD1942CAT.F_CV.VALUE, 
        ADAMS.CP1942CAT.F_CV.VALUE 
      FROM IHTREND 
  ...
  downloadCSV: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        let csvfile = '"Rain Gauge","Timestamp","Value"\r\n';
